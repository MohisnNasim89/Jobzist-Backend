...
exports.createUserProfile = async (req, res) => {
  ...
  if (userProfile) {
    throw new Error("User profile already exists. Use the update endpoint to modify it.", { status: 400 });
  }

  const { fullName } = req.body;
  if (!fullName) {
    throw new Error("Full name is required to create a user profile", { status: 400 });
  }
  ...
  if (!updatedUser) {
    throw new Error("User not found after profile creation", { status: 404 });
  }
